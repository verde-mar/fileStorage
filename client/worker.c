...
int openFile(const char *pathname, int flags){
    ...
    CHECK_OPERATION(codice == 202 && printer == 1, fprintf(stderr, "Byte scritti: %d e byte letti:%d\nNon e' stato possibile eseguire la openFile sul file %s perche' il file e' stato bloccato da un altro client:", byte_scritti, byte_letti, pathname); free(actual_request); return -1);
    CHECK_OPERATION(codice == 505 && printer == 1, fprintf(stderr, "Byte scritti: %d e byte letti:%d\nNon e' stato possibile eseguire la openFile sul file %s  perche' il file esiste gia':", byte_scritti, byte_letti, pathname); free(actual_request); return -1);
    CHECK_OPERATION(codice == 606 && printer == 1, fprintf(stderr, "Byte scritti: %d e byte letti:%d\nNon e' stato possibile eseguire la openFile sul file %s il file non esiste e non e' stato specificato O_CREATE:", byte_scritti, byte_letti, pathname); free(actual_request); return -1);
    CHECK_OPERATION(codice == 0 && printer == 1, fprintf(stdout, "Byte scritti: %d e byte letti:%d\nE' stata eseguita la openFile sul file %s con successo: ", byte_scritti, byte_letti, pathname); free(actual_request); return 0); 
...
int lockFile(const char* pathname){
    ...
    actual_request = strcat(actual_request, "lock;");
    actual_request = strcat(actual_request, pathname);
    actual_request[len] = '\0';
    ...
    int byte_letti = read_msg(fd_skt, &codice, sizeof(int)); 
    CHECK_OPERATION(byte_letti == -1 && printer != 1, return -1);
    CHECK_OPERATION(codice == 202 && printer == 1, fprintf(stderr, "Byte scritti: %d e byte letti:%d\nNon e' stato possibile eseguire la lockFile sul file %s perche' il file e' stato bloccato da un altro client:", byte_scritti, byte_letti, pathname); free(actual_request); return -1);
    CHECK_OPERATION(codice == 303 && printer == 1, fprintf(stderr, "Byte scritti: %d e byte letti:%d\nNon e' stato possibile eseguire la lockFile sul file %s  perche' perche' non puoi richiederla dopo la closeFile:", byte_scritti, byte_letti, pathname); free(actual_request); return -1);
    CHECK_OPERATION(codice == 0 && printer == 1, fprintf(stdout, "Byte scritti: %d e byte letti:%d\nE' stata eseguita la lockFile sul file %s con successo: ", byte_scritti, byte_letti, pathname); free(actual_request); return 0); 
...
int unlockFile(const char* pathname){
    ...
    actual_request = strcat(actual_request, "unlock;");
    actual_request = strcat(actual_request, pathname);
    actual_request[len] = '\0';
...
int removeFile(const char* pathname){
    ...
    actual_request = strcat(actual_request, "remove;");
    actual_request = strcat(actual_request, pathname);
    actual_request[len] = '\0';
...
int closeFile(const char* pathname){
    ...
    actual_request = strcat(actual_request, "close;");
    actual_request = strcat(actual_request, pathname);
    actual_request[len] = '\0';
...
int readFile(const char* pathname, void** buf, size_t *size){ 
    ...
    actual_request = strcat(actual_request, "read;");
    actual_request = strcat(actual_request, pathname);
    actual_request[len] = '\0';
...
int writeFile(const char* pathname, const char* dirname){
    ...
    if(dirname!=NULL){
        ...
        CHECK_OPERATION(actual_request == NULL, 
            perror("Allocazione non andata a buon fine:");
                return -1);
        actual_request = strcat(actual_request, "write;");
        actual_request = strcat(actual_request, pathname);
...
int appendToFile(const char* pathname, void* buf, size_t size, const char* dirname){
    ...
    int byte_letti = read_msg(fd_skt, &codice, sizeof(int)); 
    CHECK_OPERATION(byte_letti == -1 && printer != 1, 
        if(actual_request != NULL) free(actual_request);
            return -1);
    CHECK_OPERATION(byte_letti == 808 && printer != 1, 
        if(actual_request != NULL) free(actual_request);
            return -1);
...
int readNFiles(int N, const char* dirname){
    int byte_letti = 0, byte_scritti = 0, count = 0;
    size_t size;
    char *buf;

    int len = strlen("readN;")+1;
    char *actual_request = malloc(sizeof(char)*len);
    CHECK_OPERATION(actual_request == NULL, 
        perror("Allocazione non andata a buon fine:");
            return -1);
    
    actual_request = strcpy(actual_request, "readN;");
    actual_request[len] = '\0';
    ...
    byte_scritti += write_msg(fd_skt, &N, sizeof(int)); 
    CHECK_OPERATION(byte_scritti == -1, 
        free(actual_request);
            return -1);
    ...
    if(N>0){
        ...
        while((byte_letti = read_size(fd_skt, &size)) != -1){
